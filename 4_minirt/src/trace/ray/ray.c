...
/*   Updated: 2024/08/19 16:50:37 by daeha            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
//     if ((select != sp_obj && discriminant < 0) || (sp_obj == select && discriminant < -0.01 * vlength2(oc)))
// 		return (FALSE);
// 	if (sp_obj == select && discriminant >= -0.01 * vlength2(oc) && discriminant <= 0)
// 	{
// 		*is_outline = TRUE;
// 		//dprintf(2, "len %f, dis %f\n", vlength(ray->dir), sqrt(fabs(discriminant)));
// 	}
...
t_color3    ray_color(t_scene *scene, t_color3 color)
{
	...
	//temp_hit_outline(scene, &scene->ray, &scene->rec, &is_outline);
}
