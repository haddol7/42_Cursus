...
// 	dis.a = vdot(ray->dir, ray->dir) - (vdot(ray->dir, co->normalize) * vdot(ray->dir, co->normal));
// 	dis.half_b = vdot(ray->dir, oc) - vdot(ray->dir, co->normal) * vdot(oc, co->normal);
// 	dis.c = vdot(oc, oc) - (vdot(oc, co->normal) * vdot(oc, co->normal)) - co->radius2;
